...
internal class GeneToolWindow : SessionService
{
	...
	public override void OnStart()
	{
		...
		wnd = new DrawableWindow(800, 600);
		wnd.Text = "Gene Tool - " + ((object)hotkeyCfg.Value/*cast due to constrained. prefix*/).ToString();
		wnd.Visible = false;
		DrawableTabPane drawableTabPane = wnd.VLayout().Add(new DrawableTabPane());
		drawableTabPane.AddTab("Appearance", appearance.Root, base.Scope);
		drawableTabPane.AddTab("Personality", personality.Root, base.Scope);
	...
	private void SetGenes(params (string, float)[] genes)
	{
		...
		{
			var (unknown, value) = genes[i];
			list.Add(new GeneInfo
			{
				Id = new GeneId(unknown),
				Value = value
			});
		}
	...
	{
		base.Session.Modal.RequestInput("Type new value 0..1", "").OnResult += delegate(string result)
		{
			if (result != null)
			{
				try
