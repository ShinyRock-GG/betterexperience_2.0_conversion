...
internal class Finger
{
	...
	public Quaternion[] BestRotations { get; private set; }
	...
	public Finger(PhysicalPuppet puppet, params HumanBodyBones[] bones)
	{
		...
		BestRotations = new Quaternion[bones.Length];
		for (int i = 0; i < bones.Length; i++)
		{
			...
			BestRotations[i] = startRotations[i];
		}
	}

	public float RotateLocal(int index, Vector3 axis, float maxAngle, bool ignoreCollision)
	{
		...
		while ((double)Mathf.Abs(maxAngle) > 0.1)
		{
	...
	public bool TestCollision2(Collider c)
	{
		...
		Vector3 vector = capsuleOrientations[cc.direction];
		float num = cc.height / 2f - cc.radius;
		Vector3 position = cc.center + vector * num;
		Vector3 position2 = cc.center - vector * num;
		position = c.transform.TransformPoint(position);
		position2 = c.transform.TransformPoint(position2);
		Vector3 r = cc.transform.TransformVector(cc.radius, cc.radius, cc.radius);
		float radius = (from xyz in Enumerable.Range(0, 3)
			select (xyz != cc.direction) ? r[xyz] : 0f).Select(Mathf.Abs).Max();
