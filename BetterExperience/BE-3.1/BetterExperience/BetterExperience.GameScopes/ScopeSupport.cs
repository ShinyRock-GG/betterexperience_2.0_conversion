...
public class ScopeSupport : IDisposable
{
	...
	{
		foreach (object localObject in localObjects)
		{
			if (localObject is T)
			{
	...
	public T Lookup<T>() where T : class
	{
		...
		if (val == null)
		{
			...
			for (ScopeSupport scopeSupport = this; scopeSupport != null; scopeSupport = scopeSupport.Parent)
			{
				list.Add(scopeSupport.Name);
			}
			list.Reverse();
			throw new Exception("Unresolved service " + typeof(T).Name + " at " + string.Join("/", list));
		}
		return val;
	}

	internal void NotifyCrash(Exception e)
	{
