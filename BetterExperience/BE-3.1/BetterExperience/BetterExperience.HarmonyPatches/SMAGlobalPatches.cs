...
internal class SMAGlobalPatches
{
	...
	public static readonly Observable<bool> OnLoadaderScreenUpdate = new Observable<bool>();
	...
	{
		bool flag = ___m_showing;
		if (SCENE_LOADER_STATE.SceneLoaderStarted && SCENE_LOADER_STATE.LoadingScreenIsVisible && !flag)
		{
			SCENE_LOADER_STATE.SceneLoaderStarted = false;
		}
		if (SCENE_LOADER_STATE.LoadingScreenIsVisible != flag)
		{
			OnLoadaderScreenUpdate.Invoke(flag);
		}
		SCENE_LOADER_STATE.LoadingScreenIsVisible = flag;
	}
	...
	{
		UpdateRangeEvent updateRangeEvent = new UpdateRangeEvent();
		updateRangeEvent.Range = __result;
		OnComputeMaxVelocity.Invoke(updateRangeEvent);
		__result = updateRangeEvent.Range;
	}
	...
	public static void _OnComputeMaxDepth(ref RangeValueV2 __result, IHole hole, ParteQuePuedeEstimular estimulante, FemalePenetracionTipo tipoDePenetracio, DolorPorPenetracion dolorPorPenetracion, PlacerPorPenetraciones placerPorPenetraciones, RespiracionEngine respiracionEngine)
	{
		UpdateRangeEvent updateRangeEvent = new UpdateRangeEvent();
		updateRangeEvent.Range = __result;
		OnComputeMaxDepth.Invoke(updateRangeEvent);
		__result = updateRangeEvent.Range;
	}
	...
	{
		AutoratingInterception autoratingInterception = new AutoratingInterception();
		BeforeAutorating.Invoke(autoratingInterception);
		return !autoratingInterception.Suppress;
	}
