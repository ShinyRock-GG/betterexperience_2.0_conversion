...
public class PyStoryRuntime
{
	...
	private class CameraVision
	{
		...
		private void CaptureCameraFocusedPart()
		{
			...
			if (!BodyPartEnumHelpler.ViendoParteDelCuerpo(camera.transform.position, camera.transform.forward, 10f, ref skin9, ref hit9, ref parteDelCuerpoHumano, ref side9))
			{
				return;
			}
	...
	private LookAt scriptedLookAtTarget;
	...
	public object apply_posture(string postureId)
	{
		...
		Interaction interaction = interactionManager.CreatePostureChangeInteraction(interactionManager.CreateQueryContext(), posture);
		if (interaction == null)
		{
	...
	public object play_clip(string name, float blendingTime = -1f, List<AnimatorLayer> layers = null, AnimationCompletionMode completionMode = AnimationCompletionMode.Default)
	{
		POIPosture posture = interactionManager.CurrentPosture;
		if (name == null)
		{
			if (interactionManager.CurrentPosture.Is(poseManager.StandingPosture))
			{
				if (interactionManager.ActivePose != null)
				{
					interactionManager.AnimationController.InterruptPose("Staning requested by script");
				}
				return null;
			}
			if (interactionManager.StartAnimation("Idle") || interactionManager.StartAnimation("Binding"))
			{
				return new WaitUntil(() => !interactionManager.HasActiveInteraction);
			}
			return null;
		...
		List<PoseAnimationClip> clips = ctx.CurrentPosture.Poses.FindClips(name);
		...
		Interaction interaction = interactionManager.CreatePlayClipInteraction(ctx, name, clips, blendingTime, layers, completionMode);
		if (interactionManager.StartInteraction(interaction))
		{
			return new WaitUntil(() => !interactionManager.HasActiveInteraction);
		}
		logger.Error("Unable to start animation named {0}. See logs above for details.", name);
		return null;
	}
	...
	public InteractiveObject install_character_interaction()
	{
		...
		io.ScriptingScope = scriptingContext.ScriptingScope;
		io.transform.localScale = Vector3.one * 2f;
		return io;
	}
	...
	public IEnumerator expose_cloth(string id)
	{
		...
		if (interactions.TryObtenerSiEsValida(__InteraccionName_Ext.GetInteractionID((InteraccionSegundariaName)13), out var tmpInt))
		{
			intHandR = ((Component)(object)tmpInt).GetComponent<RopaInteractableInteraccion>();
		}
		if (interactions.TryObtenerSiEsValida(__InteraccionName_Ext.GetInteractionID((InteraccionSegundariaName)14), out tmpInt))
		{
			intHandL = ((Component)(object)tmpInt).GetComponent<RopaInteractableInteraccion>();
		}
		if (interactions.TryObtenerSiEsValida(__InteraccionName_Ext.GetInteractionID((InteraccionSegundariaName)15), out tmpInt))
		{
	...
	private IEnumerator __play_interaction(GuiaDeRopaInteractable target, RopaInteractableInteraccion interaction, GenericAgarranteObjeto agarrante, bool[] completion)
	{
		if ((UnityEngine.Object)(object)target.interactableCon != null)
		{
			...
			yield return null;
			if (interaction.interaccion.Ejecutar(int.MaxValue, -1f, ControllerPrioridadConfig.prioridad, 1f, 1f))
			{
				...
				logger.Info("W {0}", interaction.w);
				while (interaction.w < 1f)
				{
					logger.Info("W {0}", interaction.w);
					yield return null;
	...
	public IEnumerable<(string, Func<object>)> find_package_assets(string ext, bool asString = false)
	{
		ext = ext.ToLower();
	...
	public float get_required_consent(string strStimulus, string strReceiver, string strSender, bool ask = false)
	{
		...
		if (obj2 is StimulusType)
		{
			...
			case StimulusType.expose:
				...
				return ConsentNecesario.ParaConJerarquia(tde, dde, parte, sendr, ref values, personalidad, (string)null);
			}
		}
		return 0f;
	}
