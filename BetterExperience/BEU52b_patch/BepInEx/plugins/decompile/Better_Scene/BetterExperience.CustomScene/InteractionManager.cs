...
public class InteractionManager : SessionService
{
	...
	private class InteractionContextImpl : InteractionContext
	{
		...
		private void ExecuteNext()
		{
			...
			while (sequence.MoveNext())
			{
				...
				if (base.InteractionManager.logger.EnableDebug)
				{
					base.InteractionManager.logger.Debug("Executed {0}", sequence.Current.ToString());
				}
	...
	public InteractionQueryContext CreateQueryContext()
	{
	...
	public Interaction CreatePlayClipInteraction(InteractionQueryContext qCtx, string key, List<PoseAnimationClip> value, float blendingTime = -1f, List<AnimatorLayer> layers = null, AnimationCompletionMode completionMode = AnimationCompletionMode.Default)
	{
	...
	public Interaction CreatePostureChangeInteraction(InteractionQueryContext queryContext, POIPosture value)
	{
		...
		if (queryContext.activeClip != null && !queryContext.activeClip.IsIdle && !value.Is(poseManager.StandingPosture))
		{
		...
		if (value.Descriptor != null)
		{
			...
			if (value.Descriptor.Orientation != PoseOrientation.UNIVERSAL && place.Orientation != value.Descriptor.Orientation)
			{
				return null;
			}
		}
		...
		{
			interaction.Enqueue(new GotoOp(poiManager.FindPOI(interaction.TargetPosture.PoiId), PoseOrientation.UNIVERSAL));
		}
