...
{
	private class CustomPoseTracker
	{
		private enum CustomInteractionState
		{
			ACTIVATE,
			ACTIVATED,
			ACTIVE,
		...
		private CustomInteractionState State { get; set; } = CustomInteractionState.INACTIVE;
		...
		private void AfterAnimator()
		{
			...
			if (!existingCustomPose)
			{
				...
				RetargetSkeleton();
				parent.animator.Armature.ReadPose();
			}
	...
	public override void OnStart()
	{
		...
		animator = new ExtensibleAnimator(armature, PostureOffset, Gestures);
	...
	{
		Transform t = Skeleton.rootBone.FindDeepChild(bone);
		if (t == null)
		{
			logger.Error("ApplyIK: Bone {0} not found", bone);
			return;
		}
		if (ikTarget == null)
		{
			IKTargeting.SetIKTarget(t, null, smooth);
			return;
		}
		string targetName = ikTarget.Target;
		...
		IKTargeting.SetIKTarget(t, new RelIKTargeting.IKTarget
		{
			...
			LocalOffset = ikTarget.LocalPosition,
			LocalRotation = ikTarget.LocalRotation
		}, smooth);
	}
	...
	public bool StartAnimation(PoseAnimationClip clip, float blendingTime = -1f, ICollection<AnimatorLayer> layer = null, AnimationCompletionMode completionMode = AnimationCompletionMode.Default)
	{
		if (layer == null)
		{
			layer = new AnimatorLayer[1];
		}
		if (!primaryClipsById.ContainsKey(clip.UniqueName) && !idleClips.Contains(clip) && layer.Contains(AnimatorLayer.Primary))
		{
			logger.Error("Unknown clip {0} layer {1}", clip.UniqueName, layer);
			return false;
		}
		currentPose = new PoseAnimationState(animator);
		customPose.Enable();
		animator.SetAnimation(clip, layer, blendingTime, completionMode);
