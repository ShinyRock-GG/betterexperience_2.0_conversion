...
public class ComboGestureController
{
	...
	private ControlladorDeGestosFaciales faceCtl;
	...
	public ComboGestureController(GameObject go, ScopeSupport scope)
	{
		...
		faceCtl = go.GetComponentInChildren<ControlladorDeGestosFaciales>();
	...
	private void OnUpdate()
	{
		...
		{
			FaceExpressionType activeExpr = Weights.FaceExpression.MaxKey();
			TipoDeExpresion activeTipo = (TipoDeExpresion)activeExpr;
			for (int k = 0; k < Weights.FaceExpression.Keys.Length; k++)
			{
				TipoDeExpresion tipo3 = (TipoDeExpresion)k;
				float weight3 = Weights.FaceExpression[k];
				if (tipo3 == activeTipo)
				{
					faceCtl.ExagerarTipoDeExpresionPor(tipo3, faceExprTickDuration, weight3 * 10f, 3.3000002f);
				}
				else
				{
					faceCtl.SuprimirTipoDeExpresionPor(tipo3, faceExprTickDuration, weight3);
				}
				UpdateBlendShapeFaceExpr((FaceExpressionType)k, Weights.FaceExpression[k]);
			}
		}
