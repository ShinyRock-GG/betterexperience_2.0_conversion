...
internal class AnimatedArmature : AnimatedSystem
{
	...
	private IBoneDisposition runtimePose;

	private IBoneDisposition initialPose;
		...
		: base(state)
	{
		...
		InitLayer(AnimatorLayer.HandRight, "CC_Base_Upperarm.R");
		InitLayer(AnimatorLayer.HandLeft, "CC_Base_Upperarm.L");
	...
	public override void Apply(ExtensibleAnimator.AnimationClipState state, float dt)
	{
		...
		for (int i = 0; i < armaturePose.Count; i++)
		{
			...
			ComputeRotationLayer(layer, i);
			if (armaturePose.IsHip(i))
			{
				ComputeTranslationLayer(layer);
			}
			if (base.additiveLayer.TargetState != null && layer.ContainsAny(base.additiveLayer.AdditiveLayers))
			{
				...
				Quaternion tr = Quaternion.Slerp(cr, ar, base.additiveLayer.TargetState.Weight);
				runtimePose.SetRotation(i, tr);
			}
		}
		SkeletonBoneAccessors.CopyTo(runtimePose, armaturePose);
		Armature.RootBone.MapToRecursive(dt);
	}
	...
	private void ComputeRotationLayer(AnimatorLayer layer, int bone)
	{
		...
		if (weight < 1f)
		{
			...
			{
				targetRotation = Quaternion.Slerp(initialPose.GetRotation(bone), targetRotation, weight);
			}
			else
			{
				...
				{
					parentRotation = Quaternion.Slerp(parentRotation, currentState.SourceState.Disposition.GetRotation(bone), currentState.SourceState.Weight);
				}
				targetRotation = Quaternion.Slerp(parentRotation, targetRotation, weight);
			}
		}
		if (ikLayers.Contains(layer))
		{
			targetRotation = Quaternion.Slerp(targetRotation, armaturePose.GetRotation(bone), 0.99f);
		}
	...
	private void ComputeRotationLayer(BitMask<AnimatorLayer> layerMask, int bone)
	{
		for (int i = 0; i < 7; i++)
		{
	...
	public override void Update(ExtensibleAnimator.AnimationClipState state, float dt)
	{
		...
		for (int i = 0; i < seq.Indices.Length; i++)
		{
			...
			Quaternion q2 = Quaternion.Slerp(q0, q1, a);
		...
		state.Disposition.RootRotation = Quaternion.Slerp(c0.RootRotation, c1.RootRotation, a);
		...
		{
			Quaternion offset2 = Quaternion.Slerp(c0.RootMotionRotation, c1.RootMotionRotation, a);
