...
public class MonkeyHelper : MonoBehaviour
{
	...
	private IEnumerator ExecuteTransition(POIPosture posture, PoseAnimationClip clip)
	{
		...
		if (InteractionManager.CurrentPlace.POI.Id != posture.PoiId)
		{
			...
			{
				PoseAnimationClip idle = InteractionManager.AnimationController.ResolveClip("Idle");
				if (idle == null)
				{
					idle = InteractionManager.AnimationController.ResolveClip("Binding");
				}
				if (idle != null)
				{
					Interaction i = new Interaction();
					i.SourcePosture = ctx.CurrentPosture;
					i.TargetPosture = ctx.CurrentPosture;
					i.Enqueue(new AnimateOp(idle));
					...
					logger.Info("Relax to idle completed");
					yield return new WaitForSeconds(0.1f);
				}
				else
				{
					logger.Error("Failed to find Idle animation at {0}", ctx.CurrentPosture.Id);
				}
			}
		}
		if (InteractionManager.CurrentPlace.POI.Id != posture.PoiId)
		{
			...
			InteractionManager.InteractionQueryContext ctx2 = InteractionManager.CreateQueryContext();
			Interaction i2 = new Interaction();
			i2.SourcePosture = ctx2.CurrentPosture;
			i2.TargetPosture = posture;
			i2.Enqueue(new GotoOp(poi, PoseOrientation.FRONT));
			if (!InteractionManager.StartInteraction(i2))
			{
				logger.Error("Failed to start interaction");
				yield break;
			}
		...
		{
			InteractionManager.AnimationController.InterruptPose("Staning idle");
		}
	}
